...
def llm_asses(violation_path,result_path):

    #真实标注数据
    violation_file = open(violation_path, 'r')    
    violations = violation_file.readlines()
    violation_file.close()

    #大模型检测结果
    foresult_file = open(result_path, 'r')    
    ...
    return [violate_count/count,violate_true/violate_judge]
